...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            // ===== SIMPLE LINK GENERATOR WITH 1-DAY TEST OPTION =====
            if (pathname === '/gen') {
                ...
                // Plain text response with just the link (easy to copy)
                return new Response(shortLink + '\n', {
                    headers: { 'Content-Type': 'text/plain; charset=utf-8' }
                });
            }
            
            // ===== SIMPLE HTML FORM WITH 1-DAY TEST OPTION =====
            if (pathname === '/') {
...
<head>
    ...
    <style>
        ...
        .copy-btn { margin-top: 10px; padding: 8px 16px; background: #48bb78; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px; }
        .info { margin-top: 20px; padding: 15px; background: #e3f2fd; border-radius: 8px; color: #1565c0; }
        .test-badge { background: #ff6b6b; color: white; padding: 3px 8px; border-radius: 12px; font-size: 12px; margin-left: 10px; }
    </style>
</head>
<body>
    <div class="container">
        <h1>🔗 Expiring Link Generator</h1>
        <div class="info">
            <strong>Your UUID:</strong> ${yourUUID}<br>
            <strong>Permanent Link:</strong> <a href="https://${url.hostname}/${subPath}" target="_blank">https://${url.hostname}/${subPath}</a>
        </div>
        ...
        <div class="form-group">
            <label>Days until expiry <span class="test-badge">1-day test available!</span></label>
            <select id="days">
        ...
        <div class="result" id="result">
            <strong>Your Expiring Link:</strong><br>
        ...
        async function generateLink() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    document.getElementById('link').textContent = link.trim();
